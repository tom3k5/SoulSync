#!/usr/bin/env python3
"""
Generate natural voice audio files for meditation scripts using Edge TTS.

Edge TTS is a free, high-quality text-to-speech service that uses Microsoft Edge's
neural voices. It produces natural-sounding speech perfect for meditation guidance.
...
# Meditation scripts with TTS settings
MEDITATION_SCRIPTS = [
    {
        "id": "script_qhht_induction",
        "filename": "soul_remembrance.mp3",
        "voice": "en-US-AriaNeural",  # Calm, warm female voice
        "rate": "-35%",  # Very slow for meditation
        "pitch": "-5Hz",  # Slightly lower, more soothing
...
5... Eyes open. Welcome back, eternal soul. You are forever changed by this experience."""
    },
    {
        ...
        "filename": "quantum_field.mp3",
        "voice": "en-US-JennyNeural",  # Gentle, mystical female voice
        "rate": "-32%",
        "pitch": "-3Hz",
...
Ready to manifest from your soul's truth. Eyes open when you're ready."""
    ...
    {
        "id": "script_past_life",
        "filename": "past_life.mp3",
        "voice": "en-US-AriaNeural",
        "rate": "-38%",  # Extra slow for deep regression
        "pitch": "-7Hz",
...
async def generate_audio(script_data):
    """Generate audio file from script using Edge TTS."""
    output_path = OUTPUT_DIR / script_data["filename"]

    print(f"\n{'='*60}")
    print(f"Generating: {script_data['filename']}")
    print(f"Voice: {script_data['voice']}")
    print(f"Rate: {script_data['rate']}, Pitch: {script_data['pitch']}")
    print(f"{'='*60}")

    # Create communicate object with specified voice and settings
    communicate = edge_tts.Communicate(
        script_data["script"],
        script_data["voice"],
        rate=script_data["rate"],
        pitch=script_data["pitch"]
    )
...
async def generate_all_audio():
    ...
    print("""
╔══════════════════════════════════════════════════════════════════╗
║         SoulSync Meditation Audio Generator                      ║
...
╚══════════════════════════════════════════════════════════════════╝
    ...
    for script_data in MEDITATION_SCRIPTS:
        try:
            await generate_audio(script_data)
...
async def list_available_voices():
    ...
    # Filter for English voices suitable for meditation
    suitable_voices = [
        v for v in voices
        if v["Locale"].startswith("en-") and
        ("Neural" in v["Name"] or "Premium" in v["Name"])
    ]

    print(f"\nFound {len(suitable_voices)} suitable English neural voices:\n")
